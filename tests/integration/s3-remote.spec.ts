...
test("progress callbacks carry byte-level updates across both upload paths", async ({ page }) => {
  ...
  //
  // Why the phase check alone is NOT enough: `s3-uploader:progress-changed` is
...
test("static /packages handler only serves dist/, not src/ or package metadata", async () => {
  ...
  const ok = await rawGet("localhost", server.port, "/packages/s3/dist/index.js");
  expect(ok.status).toBe(200);

  const shouldBeBlocked = [
    // TypeScript source
    "/packages/s3/src/components/S3.ts",
    "/packages/s3/src/signing/sigv4.ts",
    "/packages/core/src/index.ts",
    // package metadata
    "/packages/s3/package.json",
    "/packages/s3/tsconfig.json",
    // test infrastructure
    "/packages/s3/__tests__/sigv4.test.ts",
    "/packages/s3/tests/integration/server.ts",
    // docs / license at package root
    "/packages/s3/README.md",
    "/packages/s3/LICENSE",
  ];
