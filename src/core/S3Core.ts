 ...
 * naming convention (`s3-uploader:<prop>-changed`) to build the wcBindable
...
export class S3Core extends EventTarget {
  ...
  private _setUrl(url: string): void {
    this._url = url;
    this._target.dispatchEvent(new CustomEvent("s3-uploader:url-changed", { detail: url, bubbles: true }));
  }

  private _setKey(key: string): void {
    this._key = key;
    this._target.dispatchEvent(new CustomEvent("s3-uploader:key-changed", { detail: key, bubbles: true }));
  }

  private _setEtag(etag: string): void {
    this._etag = etag;
    this._target.dispatchEvent(new CustomEvent("s3-uploader:etag-changed", { detail: etag, bubbles: true }));
  }

  private _setProgress(progress: S3Progress): void {
    this._progress = progress;
    this._target.dispatchEvent(new CustomEvent("s3-uploader:progress-changed", { detail: { ...progress }, bubbles: true }));
  }

  private _setLoading(v: boolean): void {
    this._loading = v;
    this._target.dispatchEvent(new CustomEvent("s3-uploader:loading-changed", { detail: v, bubbles: true }));
  }

  private _setUploading(v: boolean): void {
    this._uploading = v;
    this._target.dispatchEvent(new CustomEvent("s3-uploader:uploading-changed", { detail: v, bubbles: true }));
  }

  private _setCompleted(v: boolean): void {
    this._completed = v;
    this._target.dispatchEvent(new CustomEvent("s3-uploader:completed-changed", { detail: v, bubbles: true }));
  }

  private _setMetadata(m: S3ObjectMetadata | null): void {
    this._metadata = m;
    this._target.dispatchEvent(new CustomEvent("s3-uploader:metadata-changed", { detail: m ? { ...m } : null, bubbles: true }));
  }
  ...
  async requestUpload(key: string, size?: number, contentType?: string): Promise<PresignedUpload> {
    ...
    try {
      ...
      // every binding/logger subscribed to `s3-uploader:url-changed`. The Shell
  ...
  async complete(key: string, etag?: string): Promise<string> {
    ...
    try {
      ...
      // `s3-uploader:postprocess-warning` and the chain continues. Use this for
  ...
  async deleteObject(key: string): Promise<void> {
    ...
    // collision via `s3-uploader:delete-during-upload-warning` so the caller
    ...
    if (activeUploadSnapshot && this._uploading) {
      this._target.dispatchEvent(new CustomEvent("s3-uploader:delete-during-upload-warning", {
        detail: {
          key,
  ...
  async requestMultipartUpload(
    ...
    if (partCount > S3_MAX_PARTS) {
      ...
      const e = new Error(`[@csbc-dev/s3-uploader] computed part count ${partCount} exceeds S3 max ${S3_MAX_PARTS}.`);
      this._setError(e);
      this._setLoading(false);
      throw e;
    }
