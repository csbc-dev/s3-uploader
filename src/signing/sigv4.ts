...
export class SkewError extends Error {
  ...
  constructor(skewMs: number, allowedMs: number) {
    super(`[@csbc-dev/s3-uploader] clock skew ${skewMs}ms exceeds allowable ${allowedMs}ms (signer clock vs reference).`);
    this.name = "SkewError";
    this.skewMs = skewMs;
...
const subtle = (): SubtleCrypto => {
  ...
  if (!s) {
    throw new Error("[@csbc-dev/s3-uploader] Web Crypto (crypto.subtle) is required. Use Node 19+ or a modern browser.");
  }
  return s;
};
...
export async function presignS3Url(
  ...
  if (!creds.accessKeyId || !creds.secretAccessKey) {
    throw new Error("[@csbc-dev/s3-uploader] missing AWS credentials.");
  }
  if (!params.region) throw new Error("[@csbc-dev/s3-uploader] missing region.");
  if (!params.bucket) throw new Error("[@csbc-dev/s3-uploader] missing bucket.");
  if (!params.key) throw new Error("[@csbc-dev/s3-uploader] missing key.");
  ...
  if (params.extraQuery) {
    for (const k of Object.keys(params.extraQuery)) {
      if (RESERVED_SIGV4_QUERY_KEYS.has(k.toLowerCase())) {
        throw new Error(`[@csbc-dev/s3-uploader] extraQuery must not include reserved SigV4 parameter "${k}".`);
      }
    }
  }
