...
export function doPutOnce(
  ...
  return new Promise<string>((resolve, reject) => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        if (!etag) {
          ...
          reject(new MissingEtagError(
            `[@csbc-dev/s3-uploader] PUT succeeded (${xhr.status}) but response has no ETag header. Check bucket CORS 'ExposeHeaders: [\"ETag\"]' or verify the S3-compatible server emits ETag.`
          ));
          return;
        }
        if (onLoad) onLoad();
        ...
        reject(new PutHttpError(
          `[@csbc-dev/s3-uploader] PUT failed (${xhr.status}).`,
          xhr.status,
    ...
    xhr.addEventListener("error", () => {
      xhrs.delete(xhr);
      reject(new Error("[@csbc-dev/s3-uploader] network error during PUT."));
    });
    xhr.addEventListener("abort", () => {
      ...
      reject(new Error("[@csbc-dev/s3-uploader] upload aborted."));
    });
