...
export class S3 extends HTMLElement {
   ...
   * runtime method-not-found failures. (Same pattern ai-agent's `<ai-agent>`
  ...
  static wcBindable: IWcBindable = {
    ...
    properties: [
      ...
      { name: "trigger", event: "s3-uploader:trigger-changed" },
    ],
   ...
   * and re-dispatches a spurious `s3-uploader:error` event. The `{ once: true }`
  ...
  private _initRemote(): void {
    ...
    const onFail = () => {
      ...
      this._setErrorState(new Error(
        `[@csbc-dev/s3-uploader] WebSocket connection ${opened ? "lost" : "failed"}: ${url}`
      ));
  ...
  private _disposeRemote(): void {
    ...
    if (this._ws) {
      ...
      // `s3-uploader:error` to subscribers and transiently re-flips
  ...
  private _resetRemoteBusyState(): void {
    if (this._remoteValues.loading) {
      ...
      this.dispatchEvent(new CustomEvent("s3-uploader:loading-changed", { detail: false, bubbles: true }));
    }
    if (this._remoteValues.uploading) {
      this._remoteValues.uploading = false;
      this.dispatchEvent(new CustomEvent("s3-uploader:uploading-changed", { detail: false, bubbles: true }));
    }
  }
  ...
  private _setErrorState(error: unknown): void {
    ...
    this.dispatchEvent(new CustomEvent("s3-uploader:error", { detail: normalised, bubbles: true }));
  }
  ...
  _connectRemote(transport: ClientTransport): void {
    ...
    if (this._core) {
      throw new Error("[@csbc-dev/s3-uploader] _connectRemote() called while a local Core is already attached. Call disconnectedCallback / detach first.");
    }
   ...
   * cases is a silent no-op — no state change, no `s3-uploader:trigger-changed`
   ...
   * a second one, and should not leak a second `s3-uploader:trigger-changed`
  ...
  set trigger(value: boolean) {
    ...
    if (v) {
      ...
      // rising edge only).
      //
      // No `s3-uploader:trigger-changed` event is dispatched on the no-op path.
      ...
      // documented in the README's `<s3-uploader>` property table.
      if (this._currentUpload) return;
      this._trigger = true;
      this.dispatchEvent(new CustomEvent("s3-uploader:trigger-changed", { detail: true, bubbles: true }));
      this.upload().catch(() => {}).finally(() => {
        this._trigger = false;
        this.dispatchEvent(new CustomEvent("s3-uploader:trigger-changed", { detail: false, bubbles: true }));
      });
    }
  }
  ...
  private async _dispatchAsync<T>(
    ...
    if (!this._core) throw new Error("[@csbc-dev/s3-uploader] no core attached.");
  ...
  async upload(): Promise<string | null> {
    ...
    if (!file) {
      const err = new Error("[@csbc-dev/s3-uploader] file is required. Set the .file property before triggering upload.");
   ...
   * update to `_remoteValues.error` + dispatches `s3-uploader:error` along with
   * (not after) the rpc-reject delivery. Calling `_setErrorState(e)` in the
   * RPC catch on top of that produced two `s3-uploader:error` events for the
  ...
  private async _doMultipart(key: string, file: Blob, contentType: string | undefined): Promise<string | null> {
    ...
    if (this._aborted || firstError) {
      ...
      const err = firstError ?? new Error("[@csbc-dev/s3-uploader] upload aborted.");
  ...
  connectedCallback(): void {
    ...
    if (_getInternalConfig().remote.enableRemote && !this._isRemote) {
      try {
        ...
        // dispatches `s3-uploader:error` so subscribers see the transition to
