...
/**
 * <s3-callback on="completed"><script type="module">…</script></s3-callback>
 *
 * Browser-side declarative callback for state-change events emitted by an
 * ancestor <s3-uploader>. The inline ES module is loaded via a Blob URL and its
 ...
 * `<s3-callback src="https://attacker/evil.js">` and the attacker's
...
export class S3Callback extends HTMLElement {
  ...
  private _resolveEventName(on: string): string {
    ...
    if (on === "error") return "s3-uploader:error";
    return `s3-uploader:${on}-changed`;
  }

  private _findHost(): HTMLElement | null {
    const selector = this.getAttribute("for");
    if (selector) {
      // Allow callbacks placed outside the <s3-uploader> tree, e.g. in a portal.
  ...
  private async _loadModule(): Promise<void> {
    ...
    try {
      ...
      if (typeof fn !== "function") {
        ...
        this._revokeMyBlob(myBlobUrl);
        this._dispatchLocalError(new Error("[@csbc-dev/s3-uploader] <s3-callback> module has no default export function."));
        return;
      }
      ...
      // revoke it. In a page that pins a <s3-callback> for a long
  ...
  private _dispatchLocalError(error: Error): void {
    this.dispatchEvent(new CustomEvent("s3-callback:error", {
      detail: error,
      bubbles: true,
    }));
  }
  ...
  private _attach(): void {
    ...
    const handler = (e: Event) => {
      ...
      try {
        ...
        // Error shape at `s3-callback:error`.
  ...
  connectedCallback(): void {
    ...
    // Defer one microtask so the parent <s3-uploader> upgrade has run by the time
