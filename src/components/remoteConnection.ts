...
export function validateRemoteCoreUrl(url: string): URL {
  if (!url) {
    throw new Error("[@csbc-dev/s3-uploader] remote.enableRemote is true but remoteCoreUrl is empty. Set remote.remoteCoreUrl or S3_REMOTE_CORE_URL.");
  }
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
    throw new Error(`[@csbc-dev/s3-uploader] remote.remoteCoreUrl is not a valid URL: ${url}`);
  }
  if (parsed.protocol !== "ws:" && parsed.protocol !== "wss:") {
    throw new Error(`[@csbc-dev/s3-uploader] remote.remoteCoreUrl must use ws:// or wss:// scheme, got ${parsed.protocol} (${url}).`);
  }
  return parsed;
}
